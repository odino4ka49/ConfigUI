    ...
    if level_n in sample:
        ...
        if "type" in obj_sample and obj_sample["type"] == "new":
            for n in next_level:
                ...
                if "remote_attributes" in sample_l and "Class" in sample_l["remote_attributes"] and n["Class"] in \
                        sample_l["remote_attributes"]["Class"]:
        ...
        if "positioning" in sample_l["display_attributes"] and sample_l["display_attributes"][
            ...
            matrix = {"name": "Matrix", "id": parseId(node) + "matrix", "_parents": [],
                      "attributes": {"min": [], "extra": []}, "direction": direction}
            ...
            if matrix_size != 0:
                ...
                if "sort_field" in sample_l:
                    ...
                    if sample_l["display_attributes"]["matrix_type"] == "channels":
                        ...
                        for sortfield in sortfields:
                            neighbours = sorted(neighbours, key=lambda k:
                            next((attr for attr in k["attributes"]["min"] if attr["key"] == sortfield), {"value": ""})[
                                "value"])
            ...
            for sortfield in sortfields:
                neighbours = sorted(neighbours, key=lambda k:
                next((attr for attr in k["attributes"]["min"] if attr["key"] == sortfield), {"value": ""})["value"])
    return neighbours
...
def getRemoteAttributes(node, level):
    ...
    if level == "0":
        ...
        obj_sample = sample["level" + level]
    if "remote_attributes" in obj_sample and "Class" in obj_sample["remote_attributes"] and node["Class"] in \
            obj_sample["remote_attributes"]["Class"]:
        ...
        for attr in attribute_names:
            ...
            # wanted to reverse matrix here
                ...
                obj_attributes.append(
                    reverseMatrix(normalizeMatrix(getValueByPath(node, attr), node["Inputs"], node["Outputs"])))
            else:
                obj_attributes.append(
                    reverseMatrix(normalizeMatrix(getValueByPath(node, attr), node["Inputs"], node["Outputs"])))
        ...
        if remote["type"] == "matrix_equation":
            ...
            remote["width"] = (vector1["width"] + vector2["width"] + max(matrix1["width"],
                                                                         matrix2["width"]) + 8) * 9 + 30
            remote["height"] = (max(vector1["height"], vector2["height"], matrix1["height"]) + max(vector1["height"],
                                                                                                   vector2["height"],
                                                                                                   matrix2[
                                                                                                       "height"]) + max(
                vector3["height"], vector2["height"]) + 5) * 17 + 20
    else:
        return None
    return remote
...
def sortMatrixObjects(objects, sortname, size, parent_node):
    ...
    for i in range(0, size):
        ...
        if object != None:
            ...
            object = {"name": "Matrix", "id": parseId(parent_node) + str(i), "_parents": [],
                      "attributes": {"min": [{"key": "Index", "value": i}], "extra": []}, "width": width,
                      "height": height}
            sorted.append(object)
    return sorted
                        ...
                        if lc["Name"] == n["name"]:
                            ...
                            if template["components"] != None:
                                ...
                                for to_obj in lc_links:
                                    ...
                                    add_links.append({"from": n["id"], "to": to_id, "id": n["id"] + to_id,
                                                      "text": to_obj[template["component_ID"]]})
...
def findPath(tree,objid):
    def findpath(root,objid,path):
        for obj in root["_parents"]:
            if obj["id"] == objid:
                path.append(obj["name"])
                return path
            else:
                path.append(obj["name"])
                findpath(obj,objid,path)

    path = [tree["name"]]
    return findpath(tree,objid,path)
